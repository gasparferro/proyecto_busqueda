...
from .forms import Creacionformulario
from .models import Hobbits, Elfo, Enano, Orco

# En tu archivo views.py

from django.shortcuts import render
from app1.forms import Creacionformulario
from app1.models import Hobbits, Elfo, Enano, Orco

def buscar(request):
    if request.method == 'GET':
        form = Creacionformulario(request.GET)
        if form.is_valid():
            nombre = form.cleaned_data['nombre']
            genero = form.cleaned_data['genero']
            profesion = form.cleaned_data['profesion']
            
            hobbits = Hobbits.objects.filter(nombre=nombre, genero=genero, profesion=profesion)
            elfos = Elfo.objects.filter(nombre=nombre, genero=genero, profesion=profesion)
            enanos = Enano.objects.filter(nombre=nombre, genero=genero, profesion=profesion)
            orcos = Orco.objects.filter(nombre=nombre, genero=genero, profesion=profesion)
            ...
            return render(request, 'app1/resultados_busqueda.html', context)
    
    else:
        form = Creacionformulario()
    
    return render(request, 'app1/buscar.html', {'form': form})
